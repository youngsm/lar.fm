...
# def _iso_features(coord, offset, k=8, r=None):
#     # KNN in the same set
#     _, dist = pointops.knn_query(
#         min(k, 8),
#         coord.float(),
#         offset.int(),
#         coord.float(),
#         offset.int(),
#     )
#     # dist: (N, K, 1?) -> flatten to (N, K)
#     dist_flat = dist.view(dist.shape[0], -1)[..., 1:]
#     d1 = dist_flat.sqrt().median(dim=1).values.clamp_min(1e-6)
#     # d1 = dist_flat[:, 0].clamp_min(1e-6)
#     r = r or 1.0
#     return torch.log1p(r / d1).unsqueeze(-1)
...
class PointTransformerV3(PointModule):
    ...
    def forward(self, data_dict):
        ...
        # if self.add_iso_features:
        #     iso = _iso_features(point.coord, point.offset, self.iso_k, self.iso_r)
        #     point.feat = torch.cat([point.feat, iso.to(point.feat.dtype)], dim=-1)
